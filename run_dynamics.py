...
def main(_):
    ...
    data_indices = np.random.randint(
        train_size, size= [FLAGS.algo.num_ensemble, train_size],
    )
    ...
    while True:
        epoch += 1
        # Learn
        for step in tqdm(range(int(train_size / FLAGS.batch_size) * 10), smoothing= 0.1, desc= f"{epoch}"):
            batch_indices = np.random.randint(train_size, size= [FLAGS.algo.num_ensemble, FLAGS.batch_size])
