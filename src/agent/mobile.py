...
class MOBILEAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch: Batch):
        new_rng, curr_key, sample_key, action_key, target_key = jax.random.split(agent.rng, 5)

        def critic_loss_fn(critic_params):
            q1, q2 = agent.critic(batch["observations"], batch["actions"], params= critic_params)
            
            # Penalty
            mean, logvar = agent.dynamics(batch["obs_actions"][None, :].repeat(agent.config["num_ensemble"], 0))
            mean = mean.at[..., :-1].add(batch["observations"][None, :].repeat(agent.config["num_ensemble"], 0))
            ...
            samples = jax.vmap(lambda key: mean + jax.random.normal(key, shape=std.shape) * std)(sample_keys)
            
            pred_next_observations = samples[..., :-1]
            num_samples, num_ensemble, batch_size, obs_dim = pred_next_observations.shape
            
            pred_next_observations = pred_next_observations.reshape(-1, obs_dim)
            pred_next_actions, _ = agent.actor(pred_next_observations).sample_and_log_prob(seed=action_key)
            ...
            penalty = penalty.at[:int(batch_size * agent.config["dataset_ratio"])].set(0.0)
            
            next_actions, next_log_probs = agent.actor(batch['next_observations']).sample_and_log_prob(seed=target_key)
            nq1, nq2 = agent.target_critic(batch["next_observations"], next_actions)
            next_q = jnp.minimum(nq1, nq2)
            target_q = (batch["rewards"] - agent.config["penalty_coef"] * penalty.squeeze()) + agent.config["discount"] * batch["masks"] * next_q
