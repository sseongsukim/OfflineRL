...
class TD3BCAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch: Batch):
        ...
        def critic_loss_fn(critic_params):
            noise = jax.random.normal(critic_key, batch['actions'].shape) * agent.config['policy_noise']
            noise = jnp.clip(noise, -agent.config['noise_clip'], agent.config['noise_clip'])
            next_actions = agent.actor(batch['next_observations']) + noise
            next_actions = jnp.clip(next_actions, -1, 1)
            
            target_q1, target_q2 = agent.target_critic(batch['next_observations'], next_actions)
            target_q = jnp.minimum(target_q1, target_q2)
            target_q = batch['rewards'] + agent.config['discount'] * batch['masks'] * target_q
            
            current_q1, current_q2 = agent.critic(batch['observations'], batch['actions'], params=critic_params)
        ...
        new_target_critic = target_update(new_critic, agent.target_critic, agent.config['target_update_rate'])
        def update_actor():
