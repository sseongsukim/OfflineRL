...
class RAMBOAgent(flax.struct.PyTreeNode):
    ...
    def finetune_dynamics(agent, batch: Batch):
        ...
        def finetune_dynamics_loss(dynamics_params):
            inputs = batch["adv_obs_actions"][None, :].repeat(
                agent.config["num_ensemble"], 0
            )
            diff_mean, logvar = agent.dynamics(inputs, params=dynamics_params)
            diff_obs, diff_reward = diff_mean[:, :, :-1], diff_mean[:, :, -1:]
            diff_obs = diff_obs.at[..., :].add(
                batch["adv_observations"][None, :].repeat(
                    agent.config["num_ensemble"], 0
                )
            )
            mean = jnp.concatenate(
                [diff_obs, diff_reward],
                axis=-1,
            )
            std = jnp.sqrt(jnp.exp(logvar))
            ...
            # Advantage
            next_actions, _ = agent.actor(next_observations).sample_and_log_prob(
                seed=actor_key
            )
            ...
            value = (
                rewards
                + (1 - terminals.astype(float)) * agent.config["discount"] * next_q
            )
            ...
            sl_mse_loss_inv = (
                jnp.pow(sl_mean - batch["targets"], 2) * sl_inv_var
            ).mean(axis=(1, 2))
            ...
            logvar_loss = agent.config["sl_weight"] * (
                agent.dynamics(method="get_max_logvar_sum")
                - agent.dynamics(method="get_min_logvar_sum")
            )
