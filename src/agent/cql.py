...
class CQLAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch: Batch):
        ...
        def critic_loss_fn(critic_params):
            ...
            q1_policy, q2_policy = agent.critic(
                batch["observations"], policy_actions, params=critic_params
            )
            q1_random, q2_random = agent.critic(
                batch["observations"], random_actions, params=critic_params
            )
        ...
        new_temp, temp_info = agent.temp.apply_loss_fn(
            loss_fn=temp_loss_fn_partial, has_aux=True
        )
