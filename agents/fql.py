...
class FQLAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        pred = self.network.select("actor_bc_flow")(
            batch["observations"], actions=x_t, times=t, params=grad_params
        )
        ...
        actor_actions = self.network.select("actor_onestep_flow")(
            batch["observations"], actions=noises, params=grad_params
        )
    ...
    def compute_flow_actions(
        ...
        for i in range(self.config["flow_steps"]):
            ...
            vels = self.network.select("actor_bc_flow")(
                observations, actions=actions, times=t, is_encoded=True
            )
    ...
    def create(
        ...
        actor_onestep_flow_def = ActorVectorField(
            ...
            action_dim=action_dim,
            layer_norm=config["actor_layer_norm"],
            gc_encoder=encoders.get("actor_onestep_flow"),
        )
        ...
        network_info = dict(
            ...
            actor_bc_flow=(
                actor_bc_flow_def,
                (ex_observations, None, ex_actions, ex_times),
            ),
            actor_onestep_flow=(
