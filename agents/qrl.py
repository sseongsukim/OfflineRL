...
class QRLAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        # The quasimetric value function embeds observations and goals with a single shared network.
        assert ex_observations.shape[-1] == ex_goals.shape[-1], (
            "QRL embeds observations and goals with the same network, so it cannot use oracle goal representations. "
