...
class SHARSAAgent(flax.struct.PyTreeNode):
    ...
    def high_value_loss(self, batch, grad_params):
        ...
        q1, q2 = self.network.select("target_high_critic")(
            ...
            actions=batch["high_value_actions"],
        )
    ...
    def high_critic_loss(self, batch, grad_params):
        ...
        q = (
            ...
            + (self.config["discount"] ** batch["high_value_subgoal_steps"])
        ...
        if self.config["value_loss_type"] == "squared":
            ...
            critic_loss = (
                self.bce_loss(q1_logit, q).mean() + self.bce_loss(q2_logit, q).mean()
            )
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        rng, high_value_rng, high_critic_rng, high_actor_rng, low_actor_rng = (
    ...
    def sample_actions(
        ...
        n_goals = jnp.repeat(
            jnp.expand_dims(goals, 0), self.config["num_samples"], axis=0
        )
        ...
        for i in range(self.config["flow_steps"]):
            t = jnp.full(
                (self.config["num_samples"], *observations.shape[:-1], 1),
                i / self.config["flow_steps"],
            )
            vels = self.network.select("high_actor_flow")(
                n_observations, n_goals, n_subgoals, t
            )
            n_subgoals = n_subgoals + vels / self.config["flow_steps"]

        q = self.network.select("high_critic")(
            n_orig_observations, goals=n_goals, actions=n_subgoals
        ).min(axis=0)
        subgoals = n_subgoals[jnp.argmax(q)]

        # Low-level: behavioral cloning.
        actions = jax.random.normal(
            low_seed, (*observations.shape[:-1], self.config["action_dim"])
        )
        for i in range(self.config["flow_steps"]):
            t = jnp.full((*observations.shape[:-1], 1), i / self.config["flow_steps"])
            vels = self.network.select("low_actor_flow")(
                observations, subgoals, actions, t
            )
    ...
    def create(
        ...
        network_info = dict(
            ...
            target_high_critic=(
                copy.deepcopy(high_critic_def),
                (ex_observations, ex_goals, ex_goals),
            ),
            high_actor_flow=(
                high_actor_flow_def,
                (ex_observations, ex_goals, ex_goals, ex_times),
            ),
            low_actor_flow=(
                low_actor_flow_def,
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            dataset_class="SHARSADataset",  # Dataset class name.
